...
parse_line_graph_bitmap <- function(bitmap,
  ...
  dplyr::tibble(
    date = days,
    measure = ub - values*(ub - lb)
  )
}

extract_clevel_line_graphs <- function(pdf, date_start, date_end) {
  iso2c <- substr(basename(pdf), 12, 13)
  bitmaps <- extract_line_graph_bitmaps(pdf, pages = 1:2)
  parse_line_graph_bitmap(bitmaps[[1]][[1]], date_start, date_end) %>%
    ...
    dplyr::left_join(
      parse_line_graph_bitmap(bitmaps[[1]][[2]], date_start, date_end),
      by = "date"
    ) %>%
    dplyr::rename(grocery_pharmacy = .data$measure) %>%
    dplyr::left_join(
      parse_line_graph_bitmap(bitmaps[[1]][[3]], date_start, date_end),
      by = "date"
    ) %>%
    dplyr::rename(parks = .data$measure) %>%
    dplyr::left_join(
      parse_line_graph_bitmap(bitmaps[[2]][[1]], date_start, date_end),
      by = "date"
    ) %>%
    dplyr::rename(transit_stations = .data$measure) %>%
    dplyr::left_join(
      parse_line_graph_bitmap(bitmaps[[2]][[2]], date_start, date_end),
      by = "date"
    ) %>%
    dplyr::rename(workplaces = .data$measure) %>%
    dplyr::left_join(
      parse_line_graph_bitmap(bitmaps[[2]][[3]], date_start, date_end),
      by = "date"
    ) %>%
...
#'     If \code{NULL} (the default), the code will download the current PDFs
#'     from Google's website (see above). See \code{download_google_cmr_pdfs()}
#'     for downloading the PDFs.
#' @param use_all_dates Defaults to \code{FALSE}. If \code{TRUE}, it parses
#'     all PDFs from various dates available in \code{pdf_dir} and averages
#'     over the values found. This extends the time span of the data as
#'     older Google reports contain values for earlier dates. If \code{TRUE},
#'     make sure that \code{pdf_dir} points to the directory containing the
#'     date indexed subdirectories. See \code{download_google_cmr_pdfs()}
#'     for downloading the PDFs.
...
scrape_google_cmr_data <- function(daily_data = TRUE, pdf_dir = NULL,
                                   use_all_dates = FALSE, silent = FALSE,
                                   cached = FALSE) {
  ...
  parse_cmr_ctry <- function(url, parse_line_graphs = FALSE) {
    ...
    date_start <- dplyr::case_when(
      rv$date == lubridate::ymd("2020-03-29") ~ lubridate::ymd("2020-02-16"),
      rv$date == lubridate::ymd("2020-04-05") ~ lubridate::ymd("2020-02-23"),
      rv$date == lubridate::ymd("2020-04-11") ~ lubridate::ymd("2020-02-29")
    )
    date_end <- rv$date

    df <- extract_clevel_line_graphs(pdf, date_start, date_end)
    if (!silent) message("done!")
    list(rv, df)
  }
  ...
  if(cached) {
    ...
    if (use_all_dates && is.null(pdf_dir))
      stop("'use_all_dates' == 'TRUE' requires 'pdf_dir' to be set")
    ...
    if(is.null(pdf_dir)) {
      ...
      urls <- list(xml2::read_html(cmr_url) %>%
        rvest::html_nodes(css = ".download-link") %>%
        rvest::html_attr('href'))
    } else {
      if (use_all_dates) {
        pdf_dir <- list.dirs(pdf_dir)
        urls <- lapply(pdf_dir[2:length(pdf_dir)],
                          list.files, pattern = ".pdf", full.names = TRUE)
      }
      else urls <- list(list.files(path = pdf_dir, pattern = ".pdf", full.names = TRUE))
    }
    ...
    rlist <- lapply(urls, parse_urls)

    rndmean <- function(x) {
      if (lubridate::is.timepoint(x)) max(x)
      else if (all(is.na(x))) NA
      else round(mean(x, na.rm = TRUE), 2)
    }

    gcmr_c <- do.call(rbind, lapply(rlist, function(x) x[[1]])) %>%
      dplyr::arrange(.data$iso3c, .data$date)
    gcmr_cd <- do.call(rbind, lapply(rlist, function(x) x[[2]])) %>%
      dplyr::group_by(.data$iso3c, .data$date) %>%
      dplyr::summarise_at(dplyr::vars(dplyr::everything()), rndmean)

    rv <- list(gcmr_c, gcmr_cd)
